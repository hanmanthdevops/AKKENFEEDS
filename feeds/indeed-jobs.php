<?php
	...
	$indeedParam = array();
	$indeedParam['publisher'] = "Akken Cloud";
	$indeedParam['publisherurl'] = "http://www.akken.com/";
	$indeedParam['lastBuildDate'] = date("D, d M Y H:i:s");

	// Indeed requested to not include jobs that were posted more than 120 days as they do not consider them to put them in their organiz searches 
	$indeedCond = " AND DATEDIFF(CURRENT_DATE(), IF(pd.status='P',pd.posted_date,IF(p.refresh_date='0000-00-00 00:00:00',pd.posted_date,p.refresh_date))) <= 120 ";
	...
	while($drow=mysql_fetch_row($dres))
	{
		...
		while($jurow=mysql_fetch_row($jures))
		{
			...
			if($seowebapi=="Y" || validateURL($jurow[1],1,$log_file))
			{
				...
				WHERE l.serial_no=$jlsno AND TRIM(l.jobs_url!='') AND TRIM(pd.postitle)!='' AND TRIM(pd.posdesc)!='' AND pd.status IN('P','R') AND pd.posstatus NOT IN (".$rowManage[0].") ".$expiredExpr.$indeedCond;
				...
				while($jobsinfo = mysql_fetch_array($jobsres))
				{
					...
					$indeedParam['job'][$i]['title']['@cdata'] = $jobsinfo['jobtitle'];
					$indeedParam['job'][$i]['date']['@cdata'] = $jobsinfo['posted_date'];
					$indeedParam['job'][$i]['referencenumber']['@cdata'] = $companyuser."-".$jobsinfo['jobid'];
					...
					if($seowebapi=="Y")
					{
						...
						$indeedParam['job'][$i]['url']['@cdata'] = $jobsinfo['jobs_url']."/jobdetails/".$seotitle."/".$jobsinfo['jobid'];
					}
					else
					{
						$indeedParam['job'][$i]['url']['@cdata'] = $jobsinfo['jobs_url']."?wsource=$fsource&companyuser=$companyuser&jobOrdrNo=".$jobsinfo['jobid'];
					}

					$indeedParam['job'][$i]['country']['@cdata'] = $jobsinfo['country'];
					$indeedParam['job'][$i]['description']['@cdata'] = $jobsinfo['description'];

					if($companyname!="")
						$indeedParam['job'][$i]['company']['@cdata'] = $companyname;

					if($jobsinfo['city']!="")
						$indeedParam['job'][$i]['city']['@cdata'] = $jobsinfo['city'];
		
					if($jobsinfo['state']!="")
						$indeedParam['job'][$i]['state']['@cdata'] = $jobsinfo['state'];

					if($jobsinfo['postalcode']!="")
						$indeedParam['job'][$i]['postalcode']['@cdata'] = $jobsinfo['postalcode'];

					if($jobsinfo['fsalary']!="")
						$indeedParam['job'][$i]['salary']['@cdata'] = $jobsinfo['fsalary'];

					if($jobsinfo['jobtype']!="")
						$indeedParam['job'][$i]['jobtype']['@cdata'] = $jobsinfo['jobtype'];

					if($jobsinfo['category']!="")
						$indeedParam['job'][$i]['category']['@cdata'] = $jobsinfo['category'];
					...
					if($seowebapi=="Y")
					{
						if($jobsinfo['joblocation']!="")
							...
							$seotitle=strtolower(str_replace(" ","-",str_replace("/","-",$jobsinfo['jobtitle'])));
						$seotitle=preg_replace('/-$/','',preg_replace('/-+/','-',preg_replace('/[^a-zA-Z0-9\s]/','-',$seotitle)));
						$jobsUrl=$jobsinfo['jobs_url']."/jobdetails/".$seotitle."/".$jobsinfo['jobid'];
					}
					else
					{
						$jobsUrl = $jobsinfo['jobs_url']."?wsource=$fsource&companyuser=$companyuser&jobOrdrNo=".$jobsinfo['jobid'];
					}

					if($candAccount!="Y")
						$indeedParam['job'][$i]['indeed-apply-data']['@cdata'] = "indeed-apply-jobcompanyname=".urlencode($companyname)."&indeed-apply-jobid=".urlencode($companyuser."-".$jobsinfo['jobid'])."&indeed-apply-joburl=".urlencode($jobsUrl)."&indeed-apply-joblocation=".urlencode($jobsinfo['city']." ".$jobsinfo['state'])."&indeed-apply-apitoken=d6ba28f4a7260d9fc01fca7934053d2472c6c6110cdfe623b8931744ead8ca50&indeed-apply-jobtitle=".urlencode($jobsinfo['jobtitle'])."&indeed-apply-posturl=".urlencode("https://webservices.akken.com/Indeed/processApp.php")."&indeed-apply-allow-apply-on-indeed=0";
	...
	$xml = Array2XML::createXML('source', $indeedParam);
